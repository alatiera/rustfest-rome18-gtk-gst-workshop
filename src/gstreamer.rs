...
pub fn on_pipeline_message(
    ...
    // A message can contmin various kinds of information but
    ...
    match msg.view() {
        ...
        MessageView::Application(msg) => match msg.get_structure() {
            ...
            Some(s) if s.get_name() == "warning" => {
                ...
                show_error_dialog(main_window.as_ref(), false, text);
            }
            _ => (),
        },
...
pub fn start_recording(
    ...
    if let Err(_) = bin.set_state(gst::State::Playing).into_result() {
        show_error_dialog(main_window.as_ref(), false, "Failed to start recording");
        return;
    }
...
pub fn stop_recording(pipeline: Option<gst::Pipeline>) {
    // If we have no pipeline (can't really happen) just return
    let pipeline = match pipeline {
        Some(ref pipeline) => pipeline.clone(),
        None => return,
...
// FIXME: ref cycles here
pub fn take_snapshot(pipeline: Option<gst::Pipeline>, main_window: Option<gtk::ApplicationWindow>) {
    let settings = load_settings();
