...
pub fn show_settings_dialog(application: &gtk::Application) {
    ...
    // Create our settings dialog struct and store clones of all relevant widgets in there. We have
    // to clone as the widgets will still be used afterwards
    let settings_dialog = SettingsDialog(Rc::new(SettingsDialogInner {
        snapshot_directory_chooser: snapshot_directory_chooser.clone(),
        snapshot_format: snapshot_format.clone(),
        timer_entry: timer_entry.clone(),
        record_directory_chooser: record_directory_chooser.clone(),
    ...
    snapshot_directory_chooser.connect_file_set(move |_| {
        let settings_dialog = upgrade_weak!(settings_dialog_weak);
        settings_dialog.save_settings();
    });

    let settings_dialog_weak = settings_dialog.downgrade();
    snapshot_format.connect_changed(move |_| {
        let settings_dialog = upgrade_weak!(settings_dialog_weak);
        settings_dialog.save_settings();
    });

    let settings_dialog_weak = settings_dialog.downgrade();
    timer_entry.connect_value_changed(move |_| {
        let settings_dialog = upgrade_weak!(settings_dialog_weak);
        settings_dialog.save_settings();
    });

    let settings_dialog_weak = settings_dialog.downgrade();
    record_directory_chooser.connect_file_set(move |_| {
        let settings_dialog = upgrade_weak!(settings_dialog_weak);
        settings_dialog.save_settings();
    });

    let settings_dialog_weak = settings_dialog.downgrade();
    record_format.connect_changed(move |_| {
        let settings_dialog = upgrade_weak!(settings_dialog_weak);
        settings_dialog.save_settings();
    });
